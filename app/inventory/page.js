...
export default function Inventory() {
  ...
  const load = async () => {
    let q = supabase.from('products')
      .select('id, name, unit, price, cost, stock, par_level, category, is_active')
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('products').insert({
      name, unit, price: Number(price), cost: Number(cost||0), par_level: Number(par||0),
      category: cat, is_active: true
    });
    if (error) setMsg(error.message); else {
      setName(''); setUnit('unidad'); setPrice(''); setCost(''); setPar(''); setCat('General');
      setMsg('Producto creado'); load();
    }
  };
  ...
  const move = async () => {
    ...
    const newStock = moveType==='IN' ? Number(prod.stock)+q : Number(prod.stock)-q;
    const { error: e1 } = await supabase.from('inventory_movements').insert({
      product_id: prodId, type: moveType, quantity: q, reason: moveType==='IN'?'Entrada manual':'Ajuste manual'
    });
      ...
      <div className="card">
        ...
        <div className="grid cols-4">
          ...
          <select className="input" value={unit} onChange={e=>setUnit(e.target.value)}>
            <option value="unidad">unidad</option><option value="porción">porción</option>
            <option value="g">g</option><option value="ml">ml</option><option value="combo">combo</option>
          </select>
          ...
          <input className="input" placeholder="Categoría (Bebidas, Hot Dogs, ...)" value={cat} onChange={e=>setCat(e.target.value)} />
          ...
          <tbody>
            {filtered.map(r=>(
              <tr key={r.id}>
                ...
                <td>{r.name}</td>
                <td>{r.unit}</td>
                <td>B/. {Number(r.price).toFixed(2)}</td>
                <td>{Math.floor(r.stock)}</td>
                <td>{Number(r.par_level).toFixed(0)}</td>
