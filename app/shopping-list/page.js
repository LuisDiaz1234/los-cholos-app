...
export default function ShoppingList() {
  ...
  const load = async () => {
    ...
    const { data: sp } = await supabase
      .from('shopping_list')
      .select('id, product_id, suggested_qty, reason')
      .eq('target_date', target)
      .order('created_at', { ascending:false });
    if (sp && sp.length) {
      const ids = sp.map(x => x.product_id);
      const { data: p } = await supabase.from('products').select('id, name, unit').in('id', ids);
      const by = Object.fromEntries((p||[]).map(x=>[x.id, x]));
      setProds(sp.map(r => ({ ...r, product: by[r.product_id] })));
    } else setProds([]);

    // ingredientes
    const { data: si } = await supabase
      .from('shopping_list_ingredients')
      .select('id, ingredient_id, suggested_qty, reason')
      .eq('target_date', target)
      .order('created_at', { ascending:false });
    if (si && si.length) {
      const ids2 = si.map(x => x.ingredient_id);
      const { data: i } = await supabase.from('ingredients').select('id, name, unit').in('id', ids2);
      const by2 = Object.fromEntries((i||[]).map(x=>[x.id, x]));
      setIngs(si.map(r => ({ ...r, ingredient: by2[r.ingredient_id] })));
    } else setIngs([]);
  };
