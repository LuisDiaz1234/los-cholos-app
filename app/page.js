...
/* ===========================
   1) Wrapper que respeta hooks
   =========================== */
export default function POSPage() {
  const { status } = useRequireAuth();   // NUNCA retornamos antes; sólo decidimos qué mostrar
  if (status === 'loading') {
    return <main className="card">Cargando…</main>;
  }
  if (status === 'noauth') {
    // si quieres redirigir al /login ya lo hace el hook; aquí mostramos algo por si acaso
    return <main className="card">Redirigiendo a iniciar sesión…</main>;
  }
  // status === 'ok'
  return <POSInner />;
}
...
/* ===========================
   2) El POS real con todos los hooks
   =========================== */
function POSInner() {
  ...
  const checkout = async () => {
    ...
    try {
      ...
      // 2) POST al endpoint de ventas (con Bearer)
